            ...
            try {
                ...
                if (Array.isArray(imageUrls) && imageUrls.length > 0) {
                    previewContainer.innerHTML = imageUrls
                        .map(url => {
                        const decodedUrl = decodeURIComponent(url).replace(/^https?:\/\/[^/]+(\/uploads\/.*)$/, '$1');
                        return `
              ...
              `;
                ...
                if (imageUrl) {
                    const normalizedUrl = imageUrl.replace(/^https?:\/\/[^/]+(\/uploads\/.*)$/, '$1');
                    previewContainer.innerHTML = `
            <div style="position:relative; display:inline-block; margin:5px;">
              <img src="${normalizedUrl}" style="width:100px; height:100px; object-fit:cover; border-radius:4px;" alt="Current">
              <div style="position:absolute; top:2px; right:2px; background:rgba(0,0,0,0.5); color:white; padding:2px 6px; border-radius:3px; font-size:10px;">Current</div>
            </div>
          `;
                }
